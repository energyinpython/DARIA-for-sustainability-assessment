...
def main():
    ...
    methods = ['economic_topsis',
               'environmental_topsis',
               'social_topsis',
               ]
    ...
    for el, met in enumerate(methods):
        ...
        # for plot
        df_varia[met[:-7].capitalize()] = list(var)
        # for next stage of research
        df_varia_fin[met[:-7].capitalize()] = list(var)
        df_varia_fin[met[:-7].capitalize() + ' dir'] = list(dir_class)
        ...
        df_final[met[:-7].capitalize()] = list(df_results['Variability'])
        # list of directions
        df_final[r'$A_{' + str(el + 1) + '} dir$'] = list(df_results['dir list'])

    # final results to csv
    print(df_final)
    df_final.to_csv('output/' + met[-6:] + '_dims_' + diff_measure + '.csv')

    df_varia_fin = df_varia_fin.set_index('Ai')
    df_varia_fin.to_csv('output/FINAL_' + met[-6:] + '_dims_' + diff_measure + '.csv')
    ...
    plt.tight_layout()
    plt.savefig('output/' + met[-6:] + '_dims_' + diff_measure + '.png')
    plt.show()

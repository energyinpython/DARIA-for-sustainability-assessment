...
# value of variable named descending True for TOPSIS and COMET, False for VIKOR
def direction(R, descending = True):
    t, m = np.shape(R)
    direction_list = []
    dir_class = []
    # iteration over alternatives i=1, 2, ..., m
...
for el, met in enumerate(methods):
    ...
    if met == 'vikor':
        dir_list, dir_class = direction(matrix, descending = False)
    else:
        dir_list, dir_class = direction(matrix, descending = True)

...
def radar_chart(dane):
    ...
    plt.legend(dane.columns, bbox_to_anchor=(0.95, 1.05, 0.3, 0.2), loc='upper left', title='DARIA-TOPSIS')
...
def main():
    ...
    G_df = pd.read_csv('data/FINAL_topsis_dims_Gini coefficient.csv')
    G_df = G_df.set_index('Ai')

    # data with alternatives' efficiency of performance calculated for the recent period
    S_df = pd.read_csv('data/results_pref_topsis_2019.csv')
